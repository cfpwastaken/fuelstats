...
export async function GET({ request, params }: { request: Request; params: { station: string } }) {
	...
	const res = await db?.query("SELECT * FROM history WHERE timestamp >= $1 AND timestamp < $2 AND station_uuid = $3 ORDER BY timestamp DESC", [from_date.toDateString(), to_date.toDateString(), station]);
